...
/**
 * Parse Excel file for Ministry Capacity Building (indicator 4.5)
 * Expected Excel format with specific field names:
 * - Officer Name
 * - Designation
 * - Training Program Name
 * - Organizer Entity
 ...
 */
export function parseMinistryCapacityBuildingExcel(
  file: File
): Promise<ExcelParseResult> {
  return new Promise((resolve) => {
    const reader = new FileReader();
    ...
    reader.onload = (e) => {
      try {
        ...
        if (officerNameIndex === -1) missingColumns.push("Officer Name");
        if (designationIndex === -1) missingColumns.push("Designation");
        if (programNameIndex === -1) missingColumns.push("Training Program Name");
        if (organiserIndex === -1) missingColumns.push("Organizer Entity");
        if (trainingTypeIndex === -1) missingColumns.push("Mode of Training");
        if (trainingPeriodIndex === -1)
          missingColumns.push("Training Period (MM/YY)");
...
/**
 * Generate a template Excel file for Ministry Capacity Building data
 * Uses specific field names for ministry indicator 4.5
 */
export function generateMinistryCapacityBuildingTemplate(): void {
  const headers = [
    "Officer Name",
    "Designation",
    "Training Program Name",
  ...
  XLSX.writeFile(workbook, "Ministry_Capacity_Building_Template.xlsx");
}