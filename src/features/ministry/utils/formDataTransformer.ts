            ...
            if (Array.isArray(section.subsection)) {
              section.subsection.forEach((subsection: any) => {
                Object.entries(subsection).forEach(([subsectionName, subsectionData]: [string, any]) => {
                  console.log(`🔄 Processing subsection: ${sectionKey}.${subsectionName}`);
                  ...
                  if (subsectionData.submittedItems && Array.isArray(subsectionData.submittedItems)) {
                    ...
                    const fieldsPerEntry = inputIds.size > 0 ? inputIds.size : 1;
                    
                    // Generic structure detection - works for any data structure
                    if (subsectionData.submittedItems.length > 0 && Array.isArray(subsectionData.submittedItems[0])) {
                      const firstElement = subsectionData.submittedItems[0];
                      ...
                      if (subsectionData.submittedItems.length > 1) {
                        ...
                        console.log(`📦 Structure 1 detected: Array of arrays. Total entries: ${subsectionData.submittedItems.length}`);
                        transformedItems = subsectionData.submittedItems.map((itemArray: any[]) => {
                          ...
                          return itemObject;
                        }).filter((obj: any) => Object.keys(obj).length > 0); // Filter out empty objects
                      } else if (firstElement.length > 0 && firstElement[0] && typeof firstElement[0] === 'object' && firstElement[0].inputId) {
                        ...
                        if (firstElement.length > fieldsPerEntry) {
                          ...
                          console.log(`📦 Structure 2 detected: Flattened array wrapped in array. Total items: ${itemsToProcess.length}, Fields per entry: ${fieldsPerEntry}`);
                        } else {
                          ...
                          console.log(`📦 Structure 1 detected: Single entry in array format. Total items: ${firstElement.length}`);
                          const itemObject: Record<string, any> = {};
                        ...
                        console.warn(`⚠️ Warning: ${sectionKey}.${subsectionName} - Invalid or empty structure detected`);
                        transformedItems = [];
                      }
                    } else {
                      ...
                      console.log(`📦 Structure 3 detected: Single flattened array. Total items: ${itemsToProcess.length}`);
                    }
                    
                    // Generic grouping logic - works for any number of fields and entries
                    if (itemsToProcess.length > 0 && fieldsPerEntry > 0) {
                      console.log(`📊 Subsection ${subsectionName}: ${fieldsPerEntry} fields per entry, ${itemsToProcess.length} total items`);
                      
                      // Validate that items can be evenly divided (warn but still process)
                      if (itemsToProcess.length % fieldsPerEntry !== 0) {
                        console.warn(`⚠️ Warning: ${sectionKey}.${subsectionName} - Items count (${itemsToProcess.length}) is not evenly divisible by fields per entry (${fieldsPerEntry}). Processing anyway...`);
                      }
                      
                      // Generic grouping - groups items by fieldsPerEntry (works for any field count)
                      for (let i = 0; i < itemsToProcess.length; i += fieldsPerEntry) {
                        const entryItems = itemsToProcess.slice(i, i + fieldsPerEntry);
                        const itemObject: Record<string, any> = {};
                    ...
                    formData[sectionKey][subsectionName] = transformedItems;
                    console.log(`✅ Successfully loaded ${sectionKey}.${subsectionName} with ${transformedItems.length} transformed entries (structure: ${structureType})`);
