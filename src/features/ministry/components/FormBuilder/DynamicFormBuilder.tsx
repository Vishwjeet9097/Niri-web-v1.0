...
export const DynamicFormBuilder: React.FC<DynamicFormBuilderProps> = React.memo(({
  ...
  const handleSubsectionAdd = useCallback((sectionKey: string, subsectionName: string) => {
    ...
    console.log("📤 Calling onChange with path:", `${sectionKey}.${subsectionName}`);
    onChange(`${sectionKey}.${subsectionName}`, newArray);
  }, [formData, generateItemId, onChange]);

  const handleSubsectionRemove = useCallback((sectionKey: string, subsectionName: string, index: number) => {
    console.log("➖ Removing subsection:", sectionKey, subsectionName, "at index:", index);
    ...
    onChange(`${sectionKey}.${subsectionName}`, (currentValue: any) => {
      ...
      console.log("📦 Current array before removal:", currentArray, "length:", currentArray.length);
      console.log("📦 Removing item at index:", index, "from array with", currentArray.length, "items");
      ...
      console.log("📦 New array after filter:", newArray, "length:", newArray.length);
      console.log("📦 Expected length:", currentArray.length - 1, "Actual length:", newArray.length);
      
      if (newArray.length !== currentArray.length - 1) {
        console.error("❌ Array length mismatch! Expected:", currentArray.length - 1, "Got:", newArray.length);
  ...
  }, [onChange]); // Removed formData dependency - we use function update instead
                ...
                >
                  ...
                  {(() => {
                    ...
                    const yesNoValue = yesNoField 
                      ? (formData[sectionKey]?.[yesNoField.id] || '').toLowerCase().trim()
                      : null;
                      ...
                      <>
                        ...
                        {section.inputs.length > 0 && (() => {
                          ...
                          section.inputs.forEach((field: any) => {
                            ...
                            const isFileUpload = field.dataType === 'file' || 
                          ...
                          return (
                            <div className="space-y-4 sm:space-y-6">
                              ...
                              {yesNoValue === 'yes' && fileUploadFields.length > 0 && (
                              ...
                              {yesNoValue === 'no' && commentFields.length > 0 && (
                                <div className="space-y-2">
                                  {commentFields
                                    .sort((a, b) => a.sequence - b.sequence)
                                    .map((field) => {
                        ...
                        {((yesNoField && yesNoValue === 'yes') || !yesNoField) && section.subsection.length > 0 && (
                          <div className={cn(
                            ...
                            {section.subsection.map((subsection, subIndex) => {
                              ...
                              return (
                                <MinistrySubsectionForm
                                  ...
                                  onRemove={(index) => handleSubsectionRemove(sectionKey, subsectionName, index)}
                                  mode={mode}
                                  disabled={disabled || isSubmitted}
                                  submissionId={submissionId}
  ...
  for (const sectionKey of sectionKeys) {
    ...
    if (prevSection !== nextSection) {
      console.log("🔄 DynamicFormBuilder: Section reference changed for", sectionKey);
      return false; // Re-render needed
    }
    ...
    if (typeof prevSection === 'object' && typeof nextSection === 'object') {
      ...
      if (prevKeys.length !== nextKeys.length) {
        console.log("🔄 DynamicFormBuilder: Section keys changed for", sectionKey);
        return false;
      }
      
      // Check if any array values changed (subsection arrays)
      for (const key of prevKeys) {
        if (Array.isArray(prevSection[key]) || Array.isArray(nextSection[key])) {
          if (prevSection[key] !== nextSection[key]) {
            console.log("🔄 DynamicFormBuilder: Subsection array changed for", sectionKey, key);
            return false; // Re-render needed
          }
        }
      }
