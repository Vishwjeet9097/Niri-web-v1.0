...
export const MospiApproverOverviewCards = ({
  ...
  onCardTitleChange,
}: MospiApproverOverviewCardsProps = {}) => {
  const { user } = useAuth();
  ...
  const [returnedToState, setReturnedToState] = useState({
    count: 0,
    total: 36,
  });
  ...
  useEffect(() => {
    const loadOverviewData = async () => {
      try {
        ...
        const { groupedByStatus, totalSubmissions, assignedStatesCount } =
          metricsData;

        // Total count logic:
        // - MOSPI_APPROVER: Always 36
        // - MOSPI_REVIEWER: assignedStatesCount (optional, with fallback to 36)
        const total = isMospiApprover ? 36 : assignedStatesCount || 36;
        ...
        if (isMospiReviewer) {
          try {
            ...
            const submissionsData: any = await apiService.getSubmissions(
              1,
              1000
            );
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ...
            if (Array.isArray(submissionsData)) {
              ...
              Array.isArray(submissionsData.data.submissions)
            ) {
        ...
        const returnedCount =
          groupedByStatus?.RETURNED_FROM_MOSPI ||
          groupedByStatus?.REJECTED_FINAL ||
          0;
      ...
      <div>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          ...
          <OverviewCard
            ...
            onClick={() => {
              const newStatus =
                selectedStatus === "APPROVED" ? null : "APPROVED";
              setSelectedStatus(newStatus);
          ...
          {isMospiApprover && (
            <OverviewCard
              ...
              onClick={() => {
                const newStatus =
                  selectedStatus === "RETURNED_FROM_MOSPI"
                    ? null
                    : "RETURNED_FROM_MOSPI";
                setSelectedStatus(newStatus);
                setSelectedCardTitle(
                  newStatus ? "Returned to State Approver" : null
                );
                onStatusFilterChange?.(newStatus);
                onCardTitleChange?.(
                  newStatus ? "Returned to State Approver" : null
