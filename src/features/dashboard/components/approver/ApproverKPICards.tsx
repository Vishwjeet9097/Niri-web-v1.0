...
export default function ApproverKPICards() {
  ...
  useEffect(() => {
    const loadKPIs = async () => {
      try {
        setLoading(true);
        const kpiData = await apiService.getRoleKPIs("STATE_APPROVER");
        ...
        const transformedKPIs = [
          {
            ...
            flagged: kpiData.flaggedSubmissions || 0,
          },
          {
        ...
        >
          <div className="flex items-center justify-between mb-1">
            ...
            <span className="text-xs text-gray-500">
              {kpi.submitted}/{kpi.total} Submitted
            </span>
          </div>
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <span className="text-xs text-pink-700 font-semibold">
              {kpi.flagged} Submissions Flagged
            </span>
          </div>
        </div>
      ))}
