...
export const InfraDevelopmentReview = ({
  ...
  const onIndicatorStatus = async (sectionId: string, status: boolean) => {
    ...
    if (isStateApprover && status && isSubmissionFromStateApprover) {
      console.log(
        `[InfraDevelopmentReview] STATE_APPROVER accepting their own indicator ${sectionId} - accepting directly without dialog`
      );
                                  ...
                                  {shouldBeEditable("2.2") ? (
                                    <div className="space-y-1.5">
                                      ...
                                      <div className="flex items-center space-x-2 py-1">
                                        <Checkbox
                                          ...
                                          onCheckedChange={(checked) => {
                                            ...
                                            // Update noDocumentAvailable and clear files if checked
                                            const updatedArray =
                                              formDataState?.section2_2?.specializedEntityArray?.map(
                                                (entry: any, idx: number) =>
                                                  idx === index
                                                    ? {
                                                        ...entry,
                                            ...
                                            if (
                                              getFieldError(
                                                `section2_2.specializedEntityArray.${index}.files`
                                              )
                                            ) {
                                              setIndicatorValidationErrors(
                                                (prev) => {
                                      ...
                                      {item.noDocumentAvailable ? (
                                        ...
                                        <>
                                          {/* Only show ONE file (first file if multiple exist) */}
                                          {item.files &&
                                          item.files.length > 0 ? (
                                            <div className="flex flex-wrap gap-1.5">
                                              <Badge
                                              ...
                                              >
                                                ...
                                                <button
                                                  type="button"
                                                  onClick={() => {
                                                    // Clear the file (only one file allowed)
                                                    handleFilesUpdate(
                                                      "2.2",
                                                      index,
                                                      []
                                          ...
                                          {/* Show "Upload" if no file, "Replace" if file exists */}
                                          <div className="flex items-center">
                                            <input
                                              type="file"
                                              accept=".pdf,.doc,.docx"
                                              onChange={async (e) => {
                                                ...
                                                if (selectedFile) {
                                                  ...
                                                  if (uploadedFile) {
                                                    // Only allow ONE file - replace instead of add
                                                    const newFiles = [
                                                      uploadedFile,
                                                    ];
                                      ...
                                      <div className="flex items-center space-x-2 py-1">
                                        <Checkbox
                                          ...
                                          onCheckedChange={(checked) => {
                                            ...
                                            // Update noDocumentAvailable and clear files if checked
                                            const updatedArray =
                                              formDataState?.section2_3?.infraDevelopmentArray?.map(
                                                (entry: any, idx: number) =>
                                                  idx === index
                                                    ? {
                                                        ...entry,
